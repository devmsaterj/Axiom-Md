...
async function serialize(msg, conn) {
    ...
    if (msg.message) {
        ...
        try {
            ...
            if (quoted?.quotedMessage) {
                ...
                msg.quoted.download = async () => {
                    try {
                        const buffer = await downloadMediaMessage(msg.quoted, "buffer", {}, conn);
                        return buffer; // this is the buffer content
                    } catch (e) {
        ...
        conn.IAS = async (jid, buff, options = {}) => {
            let buffer;
        ...
        conn.VAS = async (jid, buff, options = {}) => {
            let buffer;
            if (options && (options.packname || options.author)) {
